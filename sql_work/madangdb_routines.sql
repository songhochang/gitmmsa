...
/*!50001 VIEW `vw_orders` AS select `customer`.`name` AS `name`,`book`.`bookname` AS `bookname`,`orders`.`orderid` AS `orderid`,`orders`.`saleprice` AS `saleprice` from ((`customer` join `orders`) join `book`) where ((`customer`.`custid` = `orders`.`custid`) and (`book`.`bookid` = `orders`.`bookid`)) */;
...
-- Dump completed on 2024-08-06 15:36:05
